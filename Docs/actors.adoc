...
Each actor follows a well-defined lifecycle from the moment it it spawned until the moment it terminates.
...
2. Stop Myself: An actor may decide to stop itself by returning an api:Behavior[struct,alias='Behavior.stopped'], causing it to transition to `Stopped`.
...
7. Supervision (fail): If the supervisor decides that the actor should _not_ be restarted (e.g. because it exhausted its permitted restart count, or the fault captured is too serious to ignore),
    the actor will transition to `Stopped` and `Terminated`, similarity to how it would have if it were stopping itself manually. #TODO: I think we want to signal in PostStop that this is because we failed, and not a manual stop#
...
- **stopping** – returning a api:Behavior[struct,alias='Behavior.stopped'] as the next behavior.
...
<2> We interpret the terminal data chunk to mean that we should now stop, and do so by returning api:Behavior[struct,alias='Behavior.stopped'].
...
When sending messages that expect a response the `ask` pattern can be used. The response will be captured in an async result type
and that way can be handled from outside of actor code. Is no response is received within the specified timeout, the response will
fail with a `api:TimeoutError[struct]`

[source]
----
include::{dir_sact_doc_tests}/ActorDocExamples.swift[tag=ask]
----
<1> `replyTo` is an actor ref of the type specified with the `for` parameter of ask and is used to receive the response
<2> Create the message that will be send to the ref that `ask` is called on
