...
by an actor reach -- we get parallelism in processing across all the users/actors, however each user/actor, is going to process
its own pieces of work in a linear fashion.

A typical example where spawning _child_ actors comes in handy is splitting up a bigger piece of work into smaller ones,
and having the parent coordinate this work being done. For example, if we wanted to process a really complex dataset such
as a data from radio telescopic observations of a black hole, and we were able to split up the work into small squares, that when connected up result in
the complete image, we could create worker actors that would take care of the individual "squares" in parallel, and once
complete send their rendered output back to the parent, which would assemble the complete picture from those partial renders.
...
trace back faults to perhaps a given users dataset or message sequence that caused a crash.

> **Rule 3: Actors can change their behavior in reaction to a message**.

An Actor's _raison d'être_ can well formulated as: encapsulating state and protecting it from the complicated concurrent world out there.
...
in changes in an actors behavior.
...
The address contains an UID which is sometimes referred to as the "Incarnation" ID. This UID is used to clarify identity in
the following situation:

- an actor is spawned under the `/user/animal/caplin` path;
  - it gains an unique identifier which is appended to it's path like this automatically: `/user/animal/caplin#34535`.
- the actor hands out a few references to itself to other actors for future use.
- the actor decides it has performed it's task and should now stop itself.
- after the actor stops, the `/user/animal` parent decides that another `caplin` should be spawned, to perform some tasks that Caplin's are good at.
