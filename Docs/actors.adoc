...
#TODO: remove this section? No need for this here perhaps?#
...
In order to understand and successfully apply the actor model for building applications a number of key principles and
assumptions about systems are best internalized beforehand.
...
----
<1> Extend the `ActorBehavior` base class #TODO: Maybe we call it ClassBehavior, so it's the class-oriented one? Want to avoid this being the only place with Actor in name of behavior#
