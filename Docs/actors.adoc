...
2. Stop Myself: An actor may decide to stop itself by returning an api:Behavior[struct,alias='Behavior.stop'], causing it to transition to `Stopped`.
...
<2> We interpret the terminal data chunk to mean that we should now stop, and do so by returning api:Behavior[struct,alias='Behavior.stop'].
