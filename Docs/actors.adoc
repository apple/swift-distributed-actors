...
"you probably already know actors!"
...
- **Rule 1**: Actors only communicate using messaging.

Building systems using actors may seem slightly odd at first, if you are used to typical "web" frameworks

The three core capabilities of actors are:
...
To explain the entire idea in one sentence:

> an actor can be seen as an accumulator of state, that for every incoming message,
returns a new behavior which based on its state may choose to act differently upon the next incoming message.

That's great, but why would you use them rather than some other low level concurrency primitive?
