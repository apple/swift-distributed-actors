...
<2> Use the `context` to attempt resolving a reference to an _alive_ actor
<3> Here we have to cast the erased `resolved` actor ref, however this is guaranteed to be safe, as otherwise the resolve would not have returned it
<4> If no actor is resolved, you may want to offer a `deadLetters` ref instead #TODO: maybe we should always do this for users... it is the semantically right thing to do -- ktoso#
