...
By default in DEBUG mode, the system operates in `inboundSerializerManifestMappings` mode (see api:Serialization.Settings[struct]),
...
NOTE: By default, Foundation's JSON, PropertyList (binary or xml) coders are supported.
+
...
The ActorSystem serialization infrastructure is based around so-called api:Serialization.Manifest[struct],
