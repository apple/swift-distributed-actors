...
----
<1> Store the context as a field inside the actor. Ensure to *never* hand the context to other threads or actors, it is strictly private to the actor itself.
<2>
...
----
<1> `spawn` has an overload which allows creating the api:Actorable[protocol] using a closure which gets the context passed in.
...
The "`myself`" reference can be obtained by using `context.myself`, and it is typed as `Myself` (a type alias to `Actor<Self>`) allowing you to pass
it anywhere where any of the actors adopted actorable protocols may be expected. Learn more about this in <<actorable_protocol_composition>>.
