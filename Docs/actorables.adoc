...
<3> The `greet(name:)` call results in *sending a message* to the greeter actor. It will execute its `greet` implementation asynchronously, and eventually reply -- which is signified by the `Reply<String>` type.

TIP: The generated sources are always created in a file named with the Actorable's name, suffixed with `+GenActor.swift`.
     E.g. generated sources for `Greeter` are located in `Greeter+GenActor.swift`.
...
- "tell" -- a uni-directional message pass; which is equivalent to a `Void` returning function.
...
<1> `spawn` has an overload that allows creating the api:Actorable[protocol] using a closure which gets the context passed in.
    *Make sure to never close over any mutable shared state, as it could break the promise of state isolation actors rely upon*.
...
When implementing functions inside an ac

==== Message Access-Control

#Access control is fairly limited, as we'd realistically want more control, e.g. using some @Actorable on methods.#

Currently, all except a few functions declared in an Actorable type are generated (by <<gen_actors, GenActors>>) as actor messages.
...
An actorable can react to lifecycle events by overriding the `preStart`, `postStop` and `receiveTerminated` functions, like this:

[source]
----
include::{dir_sact_doc_tests}/Actorable/ActorableDocExamples.swift[tag=lifecycle_callbacks]
----
<1> `preStart` executes before any other message is received by the actor, and is a good place to perform additional post-init initialization work, e.g. scheduling timers or similar.
...
The entire lifecycle is perhaps best explained using the following diagram that illustrates lifecycle transitions and signals
...
3. Stop by Parent: Alternatively, the current actor's parent could send a stop signal (using `context.stop(childRef)`), which the actor would have to abide to.
...
5. Supervision: If an actor throws an error or causes a fault it will transition to Failing 🔥, its supervisor will be invoked to see if the actor should be restarted or not.
