...
==== Mindset for Handling Failure with Actors

#TODO: Using actors, we separate happy paths, from failure paths#.

Errors which are meant to be handled in your logic -- handle them using your logic.
...
==== Thread Isolation of Errors (Default)

[cols="20,80"]
|===

| When to use | Always; Always-on and complementary to other modes.

| Effort      | Low, built in supervision of `Error` throws.
...
| When to use   | To isolate actors with a process boundary. E.g. to isolate potentially unsafe C code etc.

| Effort        | Medium; need to bootstrap application starting with `ProcessIsolated`. See #TODO NIO example# how to use with Swift NIO.

| Security note | *This is NOT a security feature.* No protection from actively malicious code is achieved by this approach.
                  It can protect from memory leaks and issues in unsafe but not malicious code.

|===

To overcome the limitations of supervising faults in present day Swift as well as isolating faults originating from potentially
unsafe code (such as C libraries which you may not have control over), the `ProcessIsolated` class offers automatic and same
"look and feel" ability to control operating system processes for isolating actor system instances from one another.

By using the api:ProcessIsolated[class], you can easily maintain multiple operating system processes.
The following diagram showcases a structure of an process using api:ProcessIsolated[class] to spawn some servant processes.

Servants are ActorSystem managed child processes (called servants, to separate them from non-managed child processes),
which partake in the same cluster as their master process.

image::images/process-isolated-servants.png[ProcessIsolated process and system structure]
...
NOTE: While this method is effective in isolating faults including serious ones like segfaults or similar in
      ...
      +
      *Having that said, this is the recommended way to achieve fault isolation with status-quo Swift.*

==== Node Isolation

|===

| When to use | To distribute your application across multiple nodes; Scale-out for either redundancy of because of memory constraints.

| Effort      | Medium; The cluster needs to be bootstrapped, and any messages to be send across nodes need to have <<Serialization, serializers>> defined.

|===

==== ☠️ *Unsafe*, PoC: Thread Isolation of Faults
...
| When to use | Never. (Almost never... could be used for asap-but-graceful-shutdown)

| Effort      | Low, though wrong -- this leads to memory leaks and is not safe for production environments.

|===


CAUTION: **The supervision of faults is an UNSAFE Proof of Concept. DO NOT USE.** +
         +
...
will be logged and handled by stopping or restarting the offending actor, however resources will leak. +
         This behavior can be controlled with the `supervisionMode` setting on api:ActorSystemSettings[struct]. +
         +
         Supervision also handles `Error` throws which is safe in the same way throws in normal Swift applications are.


WARNING: It is possible to enable a special mode in which faults _are_ supervised just as errors, however due to current
Swift limitations, this method will result in leaking memory upon "survival" of such crash. +
         +
         This mode may be enabled using the `-DSACT_DANGER_SUPERVISE_ACTOR_FAULTS` flag #TODO make this flag#,
and should be used with great care, as it is using potentially unsafe methods and _does_ leak memory upon survival of fault. +
         +
         This mode is ONLY intended to "survive crash and immediately issue a graceful shutdown" of a system.
...
5   Swift Distributed ActorsBenchmarks            0x000000010ece8188 $S24Swift Distributed ActorsBenchmarks22favouriteFruitBehaviory0A5Actor0F0OySSGSDyS2SGFAfC0G7ContextCySSG_SStcfU_ + 328
6   Swift Distributed ActorsBenchmarks            0x000000010ece870d $S24Swift Distributed ActorsBenchmarks22favouriteFruitBehaviory0A5Actor0F0OySSGSDyS2SGFAfC0G7ContextCySSG_SStcfU_TA + 13
7   Swift Distributed ActorsBenchmarks            0x000000010ece8553 $S12DistributedActors0B7ContextCySSGSSAA8BehaviorOySSGs5Error_pIegggozo_ADSSAGsAH_pIeggnozo_TR + 51
8   Swift Distributed ActorsBenchmarks            0x000000010ece876b $S12DistributedActors0B7ContextCySSGSSAA8BehaviorOySSGs5Error_pIegggozo_ADSSAGsAH_pIeggnozo_TRTA + 27
9   Swift Distributed ActorsBenchmarks            0x000000010ec8bf44 $S12DistributedActors8BehaviorO16interpretMessage7context7message4file4lineACyxGAA0B7ContextCyxG_xs12StaticStringVSutKF + 916
----
...
5	Swift Distributed ActorsBenchmarks	0x000000010ece8188	 demangled:closure #1 (DistributedActors.ActorContext<Swift.String>, Swift.String) -> DistributedActors.Behavior<Swift.String> in Swift Distributed ActorsBenchmarks.favouriteFruitBehavior([Swift.String : Swift.String]) -> DistributedActors.Behavior<Swift.String> + 328
6	Swift Distributed ActorsBenchmarks	0x000000010ece870d	 demangled:partial apply forwarder for closure #1 (DistributedActors.ActorContext<Swift.String>, Swift.String) -> DistributedActors.Behavior<Swift.String> in Swift Distributed ActorsBenchmarks.favouriteFruitBehavior([Swift.String : Swift.String]) -> DistributedActors.Behavior<Swift.String> + 13
7	Swift Distributed ActorsBenchmarks	0x000000010ece8553	 demangled:reabstraction thunk helper from @escaping @callee_guaranteed (@guaranteed DistributedActors.ActorContext<Swift.String>, @guaranteed Swift.String) -> (@owned DistributedActors.Behavior<Swift.String>, @error @owned Swift.Error) to @escaping @callee_guaranteed (@guaranteed DistributedActors.ActorContext<Swift.String>, @in_guaranteed Swift.String) -> (@owned DistributedActors.Behavior<Swift.String>, @error @owned Swift.Error) + 51
8	Swift Distributed ActorsBenchmarks	0x000000010ece876b	 demangled:partial apply forwarder for reabstraction thunk helper from @escaping @callee_guaranteed (@guaranteed DistributedActors.ActorContext<Swift.String>, @guaranteed Swift.String) -> (@owned DistributedActors.Behavior<Swift.String>, @error @owned Swift.Error) to @escaping @callee_guaranteed (@guaranteed DistributedActors.ActorContext<Swift.String>, @in_guaranteed Swift.String) -> (@owned DistributedActors.Behavior<Swift.String>, @error @owned Swift.Error) + 27
----
