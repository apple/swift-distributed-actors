...
Thanks to the Service Discovery API having various implementations, ranging from static files, DNS records to Amazon or Kubernetes APIs.
The seed node discovery can be enabled by configuring the system's api:ServiceDiscoverySettings[struct], as follows:
...
NOTE: Note that discovery is not the same as issuing `.down`, and the removal of a node from the discovery service will _not_
  ...
  +
  If you want to defer all responsibility for listing up/down nodes to an external system, you can do so as well.
  In order to handle joining nodes, configure the discovery system, and in order to hand marking nodes as down implement a api:DowningStrategy[protocol],
  in terms of the discovery mechanism of your choice. You may want to give removed nodes a certain grace period before calling `cluster.down(nodeNotInDiscoveryAnymore)`,
in order to prevent pruning nodes too aggressively, as well as consider if you want to keep or remove the membership and failure detection mechanisms offered by the cluster by default (e.g. disabling the SWIM failure detector).
