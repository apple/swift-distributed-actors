...
documentation page in order to understand in more depth the various steps and configurable pieces involved.
...
<2> When a host:port was passed in as well, we join that node -- forming a cluster, alternatively we could use automatic <<node_discovery, Service Discovery>>
<3> We're ready, we can <<spawning_actors, spawn actors>> (and <<actorable, actorables>>) and look up other node's actors using the <<receptionist, Receptionist>>

With that, we are ready to start a few instances of the application (using plain old `swift run ...`), like so:

- `swift run MySampleCluster 7337`,
- `swift run MySampleCluster 8228 7337` (which makes the second node join the first).

The system won't be doing much though -- the nodes have joined, and you could observe their metrics and gossip (on verbose logging settings).
...
Many of built-in actors operate by listening for these events and acting upon them, for example by moving workloads off leaving
members of the cluster etc.

For the purposes of the quick start example, we are only interested in learning to subscribe to these events, which we can do like this:
...
Finally, we should actually start some actors, and also have them discover their counterparts on the other members of the cluster.
In order to do this, we'll use the built-in <<receptionist, Receptionist>> actor, which handles in a type-safe and efficient way,
discovery of actors across clustered members.

The receptionist only knows about actors which explicitly register with it. We can do this in a number of ways.
A good pattern to apply here, is to let the actor _itself_ register with the receptionist when it starts, this can be done
...
we'll use the method of the-one-spawning performing the registration. We'll register it in a specific `"chat-room"`, and later
spawn another "greeter" actor (though only on one of the nodes), which will act as the "master" of the chat room, being able to
send everyone announcements and more complex things in following steps of this quickstart.
...
<3> We spawn the `greeter` and `receptionist.subscribe` to the key under which our `chatters` are being registered, it will receive their api:Receptionist.Listing[enum] whenever it changes
...
and to to: <<receptionist>> for a detailed discussion of it's API and use-cases.
...
and follow a specific lifecycle along their api:Cluster.MemberStatus[struct] as outlined on the diagram below:

image::cluster_lifecycle.svg[Cluster Member Lifecycle]

Each api:Cluster.Member[struct] initially starts in `.joining` state and may proceed through next phases of its lifecycle
it it is able to. Note that a member's status can never "go back," i.e. the it may only move forward across the `.joining
[ -> .leaving] -> .up -> .down -> .removed` lifecycle. Refer to api:Cluster.MemberStatus[struct] for detailed discussion of the statuses.
...
and potentially should become down. These suspicions are surfaced from the failure detector as api:Cluster.ReachabilityChange[struct]s,
...
*Leaders*: Some of the member status moves, may only be performed by the cluster's leader. A <<leadership, leader>> member
...
spread throughout the cluster via a gossip protocol. Refer to api:Cluster.MemberStatus[struct] for a detailed description of each state.
...
- *Node replacements*: it is possible for a node to be crash, be restarted (and thus share the _same exact_ host and port as its previous incarnation),
...
act on those events, for example my not sending more work to an `.unreachable` node, or migrating workloads away from
a member which has declared api:Cluster.MemberStatus[struct]
...
<<event_stream, EventStream>>, which always first publishes a `snapshot` of the current state of the membership, and from there
...
keep a local `var membership: Membership` and every time a new event is received `Membership.apply` it do the stored value, like this:
...
when wanting to ensure an event triggers a specific action _exactly only once_ e.g. only if a node was specifically "replaced."
...
Generally, the pattern around using membership information is quite often based around performing some action on specific
...
actively. Such API #will soon exist# exists and may be used as `system.cluster.membershipSnapshot`. Note however that no strong
guarantees are made about "how up to date" that value is, and even the moment after one has checked it, it may have already changed
...
#TODO: We are considering offering an API like `system.cluster.membershipSnapshot` which would be simple one-off access to snapshots,
see https://github.com/apple/swift-distributed-actors/issues/299 #
...
Currently only two trivial (automatic) downing strategies are provided:
...
WARNING: Beware of so called "split brain" (cluster partition) scenarios when using this method,
         as it may potentially lead to declaring a a healthy member as `.down`, however due to the
         cluster network partition, this decision never reaches the that node, and as such it may continue to keep running
...
benefit is being entirely predictable and coordination free (!). In other words, without coordinating yet given the same
...
The coordination free part is very useful, as there is no need for additional consensus rounds when a n existing leader dies,
and all members need to decide on its replacement. Without coordination, yet thanks to being able to act on a resiliently
eventually consistent replicated api:Cluster.Membership[struct].
