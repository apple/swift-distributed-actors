...
located on other nodes. Along with this, advanced failure detection and migitation mechanisms are also enabled by default.
...
TIP: All capabilities that one has learnt about actors "locally" automatically translate and work without any code-changes
...
to do so safely without consulting some form of "registry." Such registry is implemented in form
...
Failure detection in clusters enables receiving `Terminated` signals with regards to watched actors residing on nodes,
...
the watching actor gets a api:Terminated[class] signal and may react to it.
...
lifecycle state), upon this change local watchers receive a `Terminated` signal about actors that they have watched on
