...
open class ClusteredActorSystemsXCTestCase: XCTestCase {
    ...
    public func setUpPair(
        _ modifySettings: ((inout ClusterSystemSettings) -> Void)? = nil
    ) async -> (ClusterSystem, ClusterSystem) {
...
extension ClusteredActorSystemsXCTestCase {
    ...
    public func assertAssociated(
        ...
        let probe = testKit.makeTestProbe(
            .prefixed(with: "probe-assertAssociated"),
            expecting: Set<Cluster.Node>.self,
            file: file,
            line: line
        )
        defer { probe.stop() }
    ...
    public func assertNotAssociated(
        ...
        let probe = testKit.makeTestProbe(
            .prefixed(with: "assertNotAssociated-probe"),
            expecting: Set<Cluster.Node>.self
        )
        defer { probe.stop() }
    ...
    public func assertMemberStatus(
        ...
        do {
            ...
            case .notFound:
                throw testKit.error(
                    "Expected [\(system.cluster.node)] to know about [\(node)] member",
                    file: file,
                    line: line
                )
    ...
    public func assertMemberDown(_ eventStreamProbe: ActorTestProbe<Cluster.Event>, node: Cluster.Node) throws {
        ...
        guard events.first != nil else {
            throw self._testKits.first!.fail(
                "Expected to capture cluster event about \(node) being down or removed, yet none captured!"
            )
        }
    }
    ...
    public func assertLeaderNode(
        ...
        if leaderNode != expectedNode {
            throw testKit.error(
                "Expected \(reflecting: expectedNode) to be leader node on \(reflecting: system.cluster.node) but was [\(reflecting: leaderNode)]"
            )
        }
    }
}
...
extension ClusteredActorSystemsXCTestCase {
    public func resolveRef<M>(
        _ system: ClusterSystem,
        type: M.Type,
        id: ActorID,
        on targetSystem: ClusterSystem
    ) -> _ActorRef<M> {
