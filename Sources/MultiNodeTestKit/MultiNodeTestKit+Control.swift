    ...
    public struct Control<Nodes: MultiNodeNodes>: MultiNodeTestControlProtocol {
        ...
        public subscript(_ nid: Nodes) -> Cluster.Endpoint {
            guard let node = self._allEndpoints[nid.rawValue] else {
                fatalError(
                    "No node present for [\(nid.rawValue)], available: \(self._allEndpoints) (on \(self.system))"
                )
            }
...
extension MultiNodeTest.Control {
    ...
    public func checkPoint(
        ...
        log.notice(
            "Checkpoint [\(name)], wait for all other nodes to arrive at the same checkpoint ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
        )
        ...
        log.notice(
            "Checkpoint [\(name)] passed, all nodes arrived within: \(startWait - endWait) >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        )  // TODO: make it print more nicely
    }
