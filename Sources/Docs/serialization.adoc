...
The message protocol is not very complicated: it consists of two types, a `CarId` struct and an `ParkingSpotMessages` enum
which is the message protocol an actor is speaking. Note that we already made the types conform to swift:Codable[], and
since the types are simple (and composed only of other `Codable`) types, the serialization code is generated automatically.
...
The `settings` argument passed to the configuration closure is a api:ActorSystemSettings[struct], which can be used to
...
WARNING: Due to this permissive style of sending messages, it technically is possible to accidentally send a message
         intended only for local messaging to a remote actor, in which case such send would fail and an serialization
...
While `Codable` is a fine way to serialize data, sometimes you it may be necessary to use a serializer which does not
operate using the its infrastructure. Swift Distributed Actors allows, transparently, swapping in alternative serialization mechanisms
