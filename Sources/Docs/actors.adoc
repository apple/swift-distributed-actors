...
**  since actors provide you with a "illusion of single-threadedness" while scaling efficiently across cores and machines.
*  easily **scale your systems across multiple nodes**, without changing the conceptual model of the system
...
** which allows the implementation to handle all the synchronization for us, and withing the actor code we can program
    as-if the actor was single-threaded, greatly simplifying our implementations based on actors.
. **Actors can spawn other actors**, including "child actors"
** which allows us to compartmentalize and structure our applications in so-called <<supervision_tree, supervision trees>>,
...
** which allows us to model arbitrary state machines and _stateful_ logic using actors.

A key observation here is that actors lean themselves very well to writing state machines which many of your programs
already are albeit perhaps more implicitly than explicitly -- with actors you have a choice to model the behavior
transitions more explicitly _or_ using a style with multiple mutable variables and mutating them within a single behavior.
While we advertise for the first style, as enables modeling the state space such that programming mistakes are less likely,
the second style indeed leads to less boilerplate. Both styles are fully supported by Swift Distributed Actors, so you can pick and choose
which best works for a given problem.

Essentially you can view an actor as an accumulator of state, that for every incoming message, returns a new behavior which based on its state may choose to act differently
...
however they do not provide addressable identities which are able to _keep state_ and communicate over network -- and this is where actor's excel at.

In other words: actors are focused on on identity, state and distribution thereof.
...
with messages from a mailbox. It is by-design that the actor itself can not be seen nor reached from user code, this
...
actor to react to messages. The `receive` behavior comes in a few different flavours. Firstly, the `receiveMessage`
...
- stopping: returning returning a api:Behavior[enum]`.stopped` as the next behavior
...
As shown in the snippet, we can interpret states returned by the underlying logic as states the actor should transition in in its lifecycle.
...
NOTE: It is NOT possible to stop yourself by calling the `context.stop(child:)` function to stop `myself`.
      This is to not encourage another way of doing the same thing, and make stopping easier to track -- as it should
      always result from a `return .stopped` or a failure. This makes analysing existing code bases simpler, as there
      are only the two cases to look for.
...
api:Props[struct] are properties that are used to configure an actor as it is started
