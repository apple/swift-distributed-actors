...
Using actors you will be able to:
...
Actors are not aimed to solve _all_ concurrency tasks, and as a matter of fact they would very well complement an Async/Await model,
and Futures.

Actors are focused on the bigger picture -- distribution.
...
DeathWatch is similar, yet very different to <<supervision>>, since it only allows us to observe other actors fail,
and not automatically (and while retaining their identity) restart them. The two techniques combined however serve
as the cornerstone of building resilient applications with actors.
