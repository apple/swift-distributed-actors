...
bool internal_send_message(CMailbox* mailbox, void* envelope, bool is_system_message) {
  int64_t old_status = is_system_message ? try_activate(mailbox) : increment_status_activations(mailbox);
...
bool cmailbox_run(CMailbox* mailbox, void* context, void* system_context, InterpretMessageCallback interpret_message) {
  ...
  if (has_system_messages(status)) {
    ...
    processed = 1; // marker value, not a counter; meaning that we did process system messages
    ...
    while (system_message != NULL && stillAlive) {
      //printf("Processing system message...\n");
  ...
  while (message != NULL) {
    ...
    // system message bit
    processed += 0b10;
    //printf("Processing user message...\n");
