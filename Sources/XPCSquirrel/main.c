...
int main(int argc, const char *argv[]) {
    ...
    xpc_connection_t c = xpc_connection_create("com.apple.distributedactors.XPCLibService", dispatch_get_main_queue());
    xpc_connection_set_event_handler(c,  ^(xpc_object_t event) {
        printf("[CLIENT] I GOT A REPLY: %s", event);
        ...
        fprintf(fp, "[CLIENT, pid:%d,tid:%lld] Received: %s: echo=%s\n",
                pid, threadId,
