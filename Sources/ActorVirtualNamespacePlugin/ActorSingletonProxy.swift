...
// ==== ----------------------------------------------------------------------------------------------------------------
// MARK: ActorVirtualNamespaceProxy
...
/// is configurable in `ActorVirtualNamespaceSettings`. Note that if the buffer becomes full, the *oldest* message
...
/// singleton runs on. If the singleton falls on *this* node, the proxy will spawn a `ActorVirtualNamespaceManager`,
/// which manages the actual singleton actor, and obtain the ref from it. The proxy instructs the
/// `ActorVirtualNamespaceManager` to hand over the singleton whenever the node changes.
internal class ActorVirtualNamespaceProxy<Message: ActorMessage> {
    /// Settings for the `ActorVirtualNamespace`
    private let settings: ActorVirtualNamespaceSettings

    /// The strategy that determines which node the singleton will be allocated
    private let allocationStrategy: ActorVirtualNamespaceAllocationStrategy
    ...
    private var managerRef: ActorRef<ActorVirtualNamespaceManager<Message>.Directive>?

    /// Message buffer in case singleton `ref` is `nil`
    private let buffer: StashBuffer<Message>

    init(settings: ActorVirtualNamespaceSettings, allocationStrategy: ActorVirtualNamespaceAllocationStrategy, props: Props? = nil, _ behavior: Behavior<Message>? = nil) {
    ...
    private func takeOver(_ context: ActorContext<Message>, from: UniqueNode?) throws {
        ...
        self.managerRef = try context.system._spawnSystemActor(
            ...
            ActorVirtualNamespaceManager(settings: self.settings, props: self.singletonProps ?? Props(), singletonBehavior).behavior,
            props: ._wellKnown
        )
    ...
    private func updateRef(_ context: ActorContext<Message>, node: UniqueNode?) {
        ...
        case .some(let node):
            ...
            // proxy as we do today. The challenge lies in serialization, as ActorVirtualNamespaceProxy and ActorVirtualNamespaceManager are generic.
...
// ==== ----------------------------------------------------------------------------------------------------------------
// MARK: ActorVirtualNamespaceManager + logging

extension ActorVirtualNamespaceProxy {
...
// ==== ----------------------------------------------------------------------------------------------------------------
// MARK: VirtualNamespace path / address
