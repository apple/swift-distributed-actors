...
// ==== ----------------------------------------------------------------------------------------------------------------
// MARK: ActorVirtualNamespaceManager

/// Spawned as a system actor on the node where the singleton is supposed to run, `ActorVirtualNamespaceManager` manages
/// the singleton's lifecycle and stops itself after handing over the singleton.
internal class ActorVirtualNamespaceManager<Message: ActorMessage> {
    ...
    private func takeOver(_ context: ActorContext<Directive>, from: UniqueNode?) throws {
        // TODO: (optimization) tell `ActorVirtualNamespaceManager` on `from` node that this node is taking over (https://github.com/apple/swift-distributed-actors/issues/329)
    ...
    private func handOver(_ context: ActorContext<Directive>, to: UniqueNode?) throws {
        // TODO: (optimization) tell `ActorVirtualNamespaceManager` on `to` node that this node is handing off (https://github.com/apple/swift-distributed-actors/issues/329)
...
extension ActorVirtualNamespaceManager {
...
// ==== ----------------------------------------------------------------------------------------------------------------
// MARK: ActorVirtualNamespaceManager path / address
