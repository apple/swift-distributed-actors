...
extension ActorSingleton: AnyActorSingleton {
    func stop(_ system: ClusterSystem) {
        ...
        let resolveContext = TraversalResolveContext<ActorSingletonManager<Message>.Directive>(address: ._singletonManager(name: self.settings.name, on: system.cluster.uniqueNode), system: system)
