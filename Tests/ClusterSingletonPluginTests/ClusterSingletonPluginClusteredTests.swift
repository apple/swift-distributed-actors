...
final class ClusterSingletonPluginClusteredTests: ClusteredActorSystemsXCTestCase {
    ...
    func test_da_singletonByClusterLeadership_happyPath() throws {
        ...
        let ref1 = try first.singleton.host(TheSingleton.self, settings: singletonSettings) { actorSystem in
            TheSingleton(actorSystem: actorSystem)
        }
        let ref2 = try second.singleton.host(TheSingleton.self, settings: singletonSettings, GreeterSingleton("Hello-2").behavior)
        let ref3 = try third.singleton.host(TheSingleton.self, settings: singletonSettings, GreeterSingleton("Hello-3").behavior)
