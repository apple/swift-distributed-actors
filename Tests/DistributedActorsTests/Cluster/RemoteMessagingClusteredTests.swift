...
final class RemoteMessagingClusteredTests: ClusteredActorSystemsXCTestCase {
    ...
    func test_association_shouldStayAliveWhenMessageSerializationThrowsOnReceivingSide() throws {
        ...
        let probeOnRemote = self.testKit(remote).spawnTestProbe(expecting: String.self)
        let nonCodableRefOnRemoteSystem: _ActorRef<SerializationTestMessage> = try remote.spawn(
            .anonymous,
            .receiveMessage { message in
                probeOnRemote.tell("forwarded:\(message)")
                return .same
    ...
    func test_sendingToRefWithAddressWhichIsActuallyLocalAddress_shouldWork() throws {
        ...
        let localRef: _ActorRef<String> = try local.spawn(
            "local",
            .receiveMessage { message in
    ...
    func test_remoteActors_echo() throws {
        ...
        let refOnRemoteSystem: _ActorRef<EchoTestMessage> = try remote.spawn(
            "remoteAcquaintance",
            .receiveMessage { message in
    ...
    func test_sendingToNonTopLevelRemoteRef_shouldWork() throws {
        ...
        let _: _ActorRef<Int> = try local.spawn(
            "localRef",
            .setup { context in
                let child: _ActorRef<String> = try context.spawn(
                    .anonymous,
                    .receiveMessage { message in
                        probe.tell("response:\(message)")
                        return .same
    ...
    func test_sendingToRemoteAdaptedRef_shouldWork() throws {
        ...
        let _: _ActorRef<WrappedString> = try local.spawn(
            "localRef",
            .setup { context in
    ...
    func test_actorRefsThatWereSentAcrossMultipleNodeHops_shouldBeAbleToReceiveMessages() throws {
        ...
        let localRef: _ActorRef<EchoTestMessage> = try local.spawn(
            "Local",
            .receiveMessage { message in
        ...
        let remoteRef: _ActorRef<EchoTestMessage> = try remote.spawn(
            "Remote",
            .receiveMessage { message in
