...
final class ClusterAssociationTests: ClusteredNodesTestBase {
    ...
    func test_handshake_shouldNotifyOnSuccess() throws {
        let (local, remote) = self.setUpPair()
        let p = self.testTools(local).spawnTestProbe(expecting: ClusterShell.HandshakeResult.self)
    ...
    func test_association_sameAddressNodeJoin_shouldOverrideExistingNode() throws {
        try shouldNotThrow {
            ...
            let firstEventsProbe = self.testTools(first).spawnTestProbe(expecting: ClusterEvent.self)
            let secondEventsProbe = self.testTools(second).spawnTestProbe(expecting: ClusterEvent.self)
            ...
            let secondReplacementEventsProbe = self.testTools(secondReplacement).spawnTestProbe(expecting: ClusterEvent.self)
    ...
    func test_association_shouldEstablishSingleAssociationForConcurrentlyInitiatedHandshakes_incoming_outgoing() throws {
        ...
        let p7337 = self.testTools(local).spawnTestProbe(expecting: ClusterShell.HandshakeResult.self)
        let p8228 = self.testTools(remote).spawnTestProbe(expecting: ClusterShell.HandshakeResult.self)
    ...
    func test_association_shouldEstablishSingleAssociationForConcurrentlyInitiatedHandshakes_outgoing_outgoing() throws {
        ...
        let p7337 = self.testTools(local).spawnTestProbe(expecting: ClusterShell.HandshakeResult.self)
        let p8228 = self.testTools(local).spawnTestProbe(expecting: ClusterShell.HandshakeResult.self)
    ...
    func test_down_self_shouldChangeMembershipSelfToBeDown() throws {
        try shouldNotThrow {
            ...
            let localProbe = self.testTools(first).spawnTestProbe(expecting: Membership.self)
            let remoteProbe = self.testTools(second).spawnTestProbe(expecting: Membership.self)

            // we we down local on local, it should become down there:
            try self.testTools(first).eventually(within: .seconds(3)) {
                ...
                guard let selfMember = firstMembership.uniqueMember(first.cluster.node) else {
                    throw self.testTools(second).error("No self member in membership! Wanted: \(first.cluster.node)", line: #line - 1)
                }

                try self.assertMemberStatus(on: first, node: first.cluster.node, is: .down)
                guard selfMember.status == .down else {
                    throw self.testTools(first).error("Wanted self member to be DOWN, but was: \(selfMember)", line: #line - 1)
                }
            }

            // and the second node should also notice
            try self.testTools(second).eventually(within: .seconds(3)) {
                ...
                guard let localMemberObservedOnRemote = secondMembership.uniqueMember(first.cluster.node) else {
                    throw self.testTools(second).error("\(second) does not know about the \(first.cluster.node) at all...!", line: #line - 1)
                }

                guard localMemberObservedOnRemote.status == .down else {
                    throw self.testTools(second).error("Wanted to see \(first.cluster.node) as DOWN on \(second), but was still: \(localMemberObservedOnRemote)", line: #line - 1)
                }
            }
        }
    }
