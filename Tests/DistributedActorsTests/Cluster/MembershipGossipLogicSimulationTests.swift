...
final class MembershipGossipLogicSimulationTests: ClusteredActorSystemsXCTestCase {
    ...
    func gossipSimulationTest(
        ...
        self.mockPeers = try! self.systems.map { system -> ActorRef<GossipShell<Cluster.MembershipGossip, Cluster.MembershipGossip>.Message> in
    ...
    func selectLogic(_ peer: AddressableActorRef) -> MembershipGossipLogic {
        (self.logics.first { $0.localNode == peer.address.node })!
    }
