...
final class ActorMemoryTests: XCTestCase {
    ...
    func test_osx_actorShell_instanceSize() {
        #if os(OSX)
        class_getInstanceSize(ActorShell<Int>.self).shouldEqual(536) // FIXME: make it smaller
        class_getInstanceSize(ActorShell<String>.self).shouldEqual(536) // FIXME: make it smaller
        #else
