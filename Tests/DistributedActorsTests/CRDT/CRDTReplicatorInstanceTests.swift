...
final class CRDTReplicatorInstanceTests: XCTestCase {
    ...
    func test_registerOwner_shouldAddActorRefToOwnersSetForCRDT() throws {
        ...
        let ownerP = self.testTools.spawnTestProbe(expecting: CRDT.Replication.DataOwnerMessage.self)
        ...
        guard case .registered = replicator.registerOwner(dataId: id, owner: ownerP.ref) else {
            throw self.testTools.fail("Should be .registered")
        }
    ...
    func test_write_shouldAddCRDTToDataStoreIfNew_deltaMergeBoolNotApplicable() throws {
        ...
        guard case .notFound = replicator.read(id) else {
            throw self.testTools.fail("Data store should not have g1")
        }
        ...
        guard case .applied(let writeResult, let isNew) = replicator.write(id, g1.asAnyStateBasedCRDT) else {
            throw self.testTools.fail("The write operation should have been applied")
        }
        isNew.shouldBeTrue()
        ...
        guard case .data(let readResult) = replicator.read(id) else {
            throw self.testTools.fail("Data store should have g1")
        }
    ...
    func test_write_shouldFailWhenInputAndStoredTypeDoNotMatch() throws {
        ...
        guard case .inputAndStoredDataTypeMismatch(let stored) = replicator.write(id, s1.asAnyStateBasedCRDT, deltaMerge: true) else {
            throw self.testTools.fail("The write operation should have failed due to type mismatch")
        }
    ...
    func test_write_shouldAddCRDTToDataStoreIfNew_nonDeltaCRDT() throws {
        ...
        guard case .notFound = replicator.read(id) else {
            throw self.testTools.fail("Data store should not have r1")
        }
        ...
        guard let wr1 = writeResult.underlying as? CRDT.LWWRegister<Int> else {
            throw self.testTools.fail("Should be a LWWRegister<Int>")
        }
        wr1.value.shouldEqual(r1.value)

        // Value in the data store should also match r1
        guard case .data(let readResult) = replicator.read(id) else {
            throw self.testTools.fail("Data store should have r1")
        }
    ...
    func test_write_shouldUpdateCRDTInDataStoreUsingMerge_nonDeltaCRDT() throws {
        ...
        guard case .data(let readResult) = replicator.read(id) else {
            throw self.testTools.fail("Data store should have r1b")
        }
        guard let rr1 = readResult.underlying as? CRDT.LWWRegister<Int> else {
            throw self.testTools.fail("Should be a LWWRegister<Int>")
        }
    ...
    func test_writeDelta_shouldFailIfCRDTIsNotInDataStore() throws {
        ...
        guard case .missingCRDTForDelta = replicator.writeDelta(id, g1.delta!.asAnyStateBasedCRDT) else { // ! safe because `increment` should set `delta`
            throw self.testTools.fail("The writeDelta operation should have failed because CRDT does not exist")
        }
    }
    ...
    func test_writeDelta_shouldApplyDeltaToExistingDeltaCRDT() throws {
        ...
        guard case .applied(let writeResult) = replicator.writeDelta(id, g1.delta!.asAnyStateBasedCRDT) else { // ! safe because `increment` should set `delta`
            throw self.testTools.fail("The writeDelta operation should have been applied")
        }
    ...
    func test_writeDelta_shouldFailIfNotDeltaCRDT() throws {
        ...
        guard case .cannotWriteDeltaForNonDeltaCRDT = replicator.writeDelta(id, r1.asAnyStateBasedCRDT) else {
            throw self.testTools.fail("The writeDelta operation should have failed because r1 is not delta-CRDT")
        }
    }
    ...
    func test_read_shouldFailIfCRDTIsNotInDataStore() throws {
        ...
        guard case .notFound = replicator.read(id) else {
            throw self.testTools.fail("The read operation should return .notFound")
        }
    }
    ...
    func test_delete_shouldRemoveCRDTFromDataStore() throws {
        ...
        guard case .applied = replicator.delete(id) else {
            throw self.testTools.fail("The delete operation should have been applied")
        }

        // Ensure g1 no longer exists
        guard case .notFound = replicator.read(id) else {
            throw self.testTools.fail("g1 should have been deleted")
        }
    }
}
