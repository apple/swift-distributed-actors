...
struct ClusterSingletonPluginClusteredTests {
    ...
    func test_singletonByClusterLeadership_happyPath() async throws {
        ...
        try await self.assertSingletonRequestReply(testCase, first, singleton: ref1, greetingName: "Alice", expectedPrefix: "Hello-1 Alice!")
        try await self.assertSingletonRequestReply(testCase, second, singleton: ref2, greetingName: "Bob", expectedPrefix: "Hello-1 Bob!")
        try await self.assertSingletonRequestReply(testCase, third, singleton: ref3, greetingName: "Charlie", expectedPrefix: "Hello-1 Charlie!")
    }
