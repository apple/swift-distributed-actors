...
final class MembershipGossipLogicSimulationTests: ClusteredActorSystemsXCTestCase {
    ...
    func logic(_ id: String) -> MembershipGossipLogic {
        guard let logic = (self.logics.first { $0.localNode.endpoint.systemName == id }) else {
    ...
    func test_avgRounds_manyNodes() async throws {
        ...
        try self.gossipSimulationTest(
            ...
            stopRunWhen: { (logics, _) in
                ...
                logics.allSatisfy { $0.latestGossip.converged() } && logics.allSatisfy { $0.latestGossip.membership.count(withStatus: .up) == allSystems.count }
            },
            assert: { results in
