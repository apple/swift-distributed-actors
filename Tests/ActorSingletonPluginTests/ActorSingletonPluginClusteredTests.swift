...
final class ActorSingletonPluginClusteredTests: ClusteredActorSystemsXCTestCase {
    ...
    func test_da_singletonByClusterLeadership_happyPath() async throws {
        var singletonSettings = ActorSingletonSettings()
        ...
        let ref2 = try await second.singleton.host(name: name, settings: singletonSettings) { actorSystem in
            await TheSingleton("Hello-2", actorSystem: actorSystem)
        }
        let ref3 = try await third.singleton.host(name: name, settings: singletonSettings) { actorSystem in
            await TheSingleton("Hello-3", actorSystem: actorSystem)
        }
...
//        let replyProbe1 = await self.testKit(first).makeTestProbe(expecting: String.self)
//        let ref1 = try first.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-1").behavior)
//        ref1.tell(.greet(name: "Alice-1", replyTo: replyProbe1.ref))
//
//        let replyProbe2 = await self.testKit(second).makeTestProbe(expecting: String.self)
//        let ref2 = try second.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-2").behavior)
//        ref2.tell(.greet(name: "Bob-2", replyTo: replyProbe2.ref))
//
//        let replyProbe3 = await self.testKit(third).makeTestProbe(expecting: String.self)
//        let ref3 = try third.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-3").behavior)
...
//
//        // Bring up `ActorSingletonProxy` before setting up cluster (https://github.com/apple/swift-distributed-actors/issues/463)
//        let ref1 = try await first.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-1").behavior)
//        let ref2 = try await second.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-2").behavior)
//        let ref3 = try await third.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-3").behavior)
//        _ = try await fourth.singleton.host(GreeterSingleton.Message.self, settings: singletonSettings, GreeterSingleton("Hello-4").behavior)
//
//        first.cluster.join(node: second.cluster.uniqueNode.node)
//        third.cluster.join(node: second.cluster.uniqueNode.node)
//
...
distributed actor TheSingleton: ClusterSingletonProtocol {
    ...
    init(_ greeting: String, actorSystem: ActorSystem) async {
        self.greeting = greeting
